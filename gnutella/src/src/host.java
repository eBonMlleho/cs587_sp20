...
public class host {
	...
	public static void main(String[] args) throws IOException {
		...
		// create a new UDP socket to receive packets. host port is at 7777.
		...
		while (true) {
			...
			if (received.substring(0, 4).equals("PING")) {
				...
				if (!KnownNodelist.contains(portNfromPing)) {
					...
					PingHandler pinghandler = new PingHandler(7777, portNfromPing, AvailableFileLists.get(0), TTL);
					Thread h = new Thread(pinghandler);
					h.start();
				}
				// PONG back
				PongHandler handler = new PongHandler(7777, portNfromPing);
...
//				
//					for (int i = 0; i < KnownNodelist.size(); i++) {
//						if (KnownNodelist.get(i) != portNfromPing) {
//							PingHandler Peerpinghandler = new PingHandler(portNfromPing, KnownNodelist.get(i),
//									peerfileName, TTL);
